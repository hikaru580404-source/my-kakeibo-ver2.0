...
function renderBoxOverview() {
  ...
  ['A','B','C'].forEach(box => {
    ...
    el.innerHTML += `<div class="bov-card ${box.toLowerCase()}box">
      <div class="bov-label">${box}箱</div>
      <div class="bov-budget">${hasBud ? fmt(budget) : '予算未設定'}</div>
      <div class="bov-actual">${fmt(actual)}</div>
      <div class="bov-pct ${status}">${hasBud ? pct.toFixed(0) + '%' : '─'}</div>
    </div>`;
  });
}
...
function renderBudgetList(box) {
  ...
  BOX_CATEGORIES[box].forEach(cat => {
    ...
    listEl.innerHTML += `<div class="budget-cat-row">
      <span class="budget-cat-name">${cat.replace(/（[ABC]箱）$/, '')}</span>
      <span class="budget-cat-actual">${actual > 0 ? '実績:' + fmt(actual) : ''}</span>
      <div class="budget-input-wrap">
        <span class="budget-yen">¥</span>
        <input type="number" class="budget-input" data-box="${box}" data-cat="${cat}"
          value="${budAmt > 0 ? budAmt : ''}" placeholder="予算額" min="0" />
      </div>
      <div class="budget-bar-col">
        <div class="budget-bar-wrap"><div class="budget-bar ${status}" style="width:${hasBud ? pct.toFixed(1) : 0}%;"></div></div>
        <div class="budget-pct ${status}">${hasBud ? pct.toFixed(0) + '%' : '─'}</div>
      </div>
    </div>`;
  });

  const totalEl = document.getElementById(`boxTotal${box}`);
  if (totalEl) totalEl.textContent = boxBudget > 0
    ? `${fmt(boxTotal)} / ${fmt(boxBudget)}`
    : `実績: ${fmt(boxTotal)}`;
}

function renderAll() {
  updateMonthLabel(); populateMonthJump();
  renderBoxOverview(); ['A','B','C'].forEach(renderBudgetList);
}
...
async function executeCopy() {
  ...
  const curKey   = monthKey(viewYear, viewMonth);
  const actuals  = getActuals(prevY, prevM);

  showLoading();
  document.getElementById('copyConfirmModal').style.display = 'none';
...
async function init() {
  ...
  document.getElementById('copyFromPrevBtn')?.addEventListener('click', () => {
    document.getElementById('copyConfirmModal').style.display = 'flex';
  });
  document.getElementById('copyModalCancelBtn')?.addEventListener('click', () => {
    document.getElementById('copyConfirmModal').style.display = 'none';
  });
