...
const BOX_CATEGORIES = {
  ...
  B: ['個人サブスク・通信費（B箱）','飲食費（ランチ・カフェ）（B箱）','美容・被服費（B箱）',
      '趣味・娯楽費（B箱）','個人交際費・予備費（B箱）','日用品（B箱）',
      '自己研鑽（B箱）','健康・医療（B箱）','交通費（B箱）','保険（B箱）','投資（B箱）','その他（B箱）'],
...
const PM_LABEL   = { qr_code:'QRコード決済', credit_card:'クレカ', cash:'現金', bank_in:'銀行口座', transfer_to_cash:'ATM振替' };
...
let viewYear  = _jstNow.year;
let viewMonth = _jstNow.month;
...
function updateMonthLabel() {
  ...
  if (el) el.textContent = `${viewYear}年${padZ(viewMonth)}月`;
}
...
function populateMonthJump() {
  ...
  const nowY   = jstNow.getUTCFullYear();
  const nowM   = jstNow.getUTCMonth() + 1;
  for (let i = 0; i < 24; i++) {
    let m = nowM - i, y = nowY;
    while (m <= 0) { m += 12; y--; }
    const key = monthKey(y, m);
    const opt = document.createElement('option');
    opt.value = key; opt.textContent = `${y}年${padZ(m)}月`;
...
function renderKPI() {
  ...
  const safeSet = (id, val, style) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.textContent = val;
    if (style) Object.assign(el.style, style);
  };
  safeSet('skIncome',  fmt(income));
  safeSet('skExpense', fmt(expense));
  safeSet('skBalance', fmtS(balance), { color: balance >= 0 ? 'var(--clr-income-dark)' : 'var(--clr-expense-dark)' });
}
...
function renderBoxSummary() {
  ...
  ['A','B','C'].forEach(box => {
    ...
    bList.innerHTML += `<div class="box-summary-item">
      <div class="box-summary-top">
        <span class="box-badge ${box.toLowerCase()}box">${box}</span>
        <span class="box-summary-name">${BOX_NAMES[box]}</span>
        <div class="box-summary-amounts">
          <span class="box-actual">${fmt(actual)}</span>
          ${hasBudget ? `<span class="box-budget">/ ${fmt(budget)}</span>` : '<span class="box-budget">予算未設定</span>'}
        </div>
      </div>
      <div class="box-bar-wrap"><div class="box-bar ${status}" style="width:${pct.toFixed(1)}%;background:${BOX_COLORS[box]};"></div></div>
      <div class="box-pct-label">${hasBudget ? `${pct.toFixed(1)}%消化` : '─'}</div>
    </div>`;
  });
}
  ...
  ['A','B','C'].forEach(box => {
    ...
    BOX_CATEGORIES[box].forEach(cat => {
      ...
      html += `<div class="cat-detail-row">
        <span class="cat-detail-name">${cat.replace(/（[ABC]箱）$/, '')}</span>
        <div class="cat-detail-bar-wrap"><div class="cat-detail-bar" style="width:${hasBudget ? pct.toFixed(1) : 0}%;background:${BOX_COLORS[box]};"></div></div>
        <span class="cat-detail-budget">${hasBudget ? fmt(budget) : '─'}</span>
        <span class="cat-detail-actual ${status === 'over' ? 'over' : 'ok'}">${actual > 0 ? fmt(actual) : '─'}</span>
      </div>`;
    });
    html += '</div>';
    el.innerHTML += html;
...
function renderTrendChart() {
  ...
  trendChartInst = new Chart(ctx, {
    ...
    data: { labels, datasets: [
      { label: '収入', data: incomes,  backgroundColor: 'rgba(16,185,129,.75)' },
      { label: '支出', data: expenses, backgroundColor: 'rgba(239,68,68,.75)'  }
    ]},
...
function renderPmTrendChart() {
  ...
  const datasets = pmTypes.map((pm, idx) => ({
    label: PM_LABEL[pm],
    data:  months.map(d =>
      allTx.filter(t => t.date && t.date.startsWith(monthKey(d.y, d.m)) && t.type === 'expense' && t.payment_method === pm)
...
function renderBalanceTrendChart() {
  ...
  const labels      = months.map(d => `${d.y === viewYear ? '' : d.y + '年'}${d.m}月`);
  const bankData    = [], cashData = [], totalData = [];
  ...
  months.forEach(d => {
    ...
    allTx.filter(t => t.date && t.date.startsWith(key)).forEach(tx => {
      const amt = Number(tx.amount);
      if (tx.type === 'income') {
        if (tx.payment_method === 'cash') cash += amt; else bank += amt;
      } else if (tx.type === 'expense') {
        if (tx.payment_method === 'cash') cash -= amt;
        else if (tx.payment_method === 'credit_card') { /* スルー */ }
        else bank -= amt;
      } else if (tx.type === 'transfer') { bank -= amt; cash += amt; }
    });
    bankData.push(bank); cashData.push(cash); totalData.push(bank + cash);
  });
  ...
  balanceTrendChartInst = new Chart(ctx, {
    type: 'line',
    data: { labels, datasets: [
      { label:'銀行口座', data: bankData,  borderColor:'#3b82f6', backgroundColor:'rgba(59,130,246,.1)', fill:true, tension:.3 },
      { label:'現金',     data: cashData,  borderColor:'#10b981', backgroundColor:'rgba(16,185,129,.1)', fill:true, tension:.3 },
      { label:'合計資産', data: totalData, borderColor:'#6366f1', backgroundColor:'transparent', borderDash:[5,5], tension:.3 }
    ]},
...
function renderClosingHistory() {
  ...
  if (!listEl) return;
  const sorted = [...closings].sort((a, b) => b.month.localeCompare(a.month));
  if (sorted.length === 0) {
    listEl.innerHTML = '<div style="padding:15px;text-align:center;color:var(--clr-text-muted);font-size:.85rem;">棚卸しの履歴はありません</div>';
    return;
  }
  let html = '';
  sorted.forEach(c => {
    ...
    html += `<div class="closing-history-item">
...
function renderAll() {
  updateMonthLabel(); populateMonthJump(); renderKPI();
  renderBoxSummary(); renderCatDetail();
  renderTrendChart(); renderPmTrendChart(); renderBalanceTrendChart();
...
async function init() {
  ...
  try {
    ...
    document.getElementById('monthJump')?.addEventListener('change', e => {
      ...
      viewYear = y; viewMonth = m; renderAll();
    });
    renderAll();
  } catch (err) {
    console.error('[summary init]:', err);
  } finally {
    hideLoading();
  }
}
